...
3. **F1-Score**: A standardized score to evaluate the performance of the model.
...
Because these scripts process the entire dataset and load the DINOv2 model, they require significant compute power. Do not run them on the login node! Instead, submit them as batch jobs using the provided SLURM wrappers:

```bash
# Submit Error Analysis
sbatch error_analysis_job_solution.sh

# Submit Hyperparameter Tuning
sbatch hyperparameter_tuning_job_solution.sh
```
